...
function describe(item: MenuItem | ComboDeal): string {
  if ("items" in item) {
...
function updateItem<T extends MenuItem>(item: T, changes: Partial<T>): T {
  return { ...item, ...changes };
}
...
// kitchenTicket(brownie).name = "Something else"; - Read only type, cannot assign to 'name' because it is a read-only property (immutable).
